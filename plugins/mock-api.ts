...
export default defineNuxtPlugin(() => {
  if (process.client) {
    ...
    window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      // Only intercept API calls, but exclude real backend routes (auth, users, contact)
      if (url.includes("/api/") && !url.includes("/api/auth/") && !url.includes("/api/users/") && !url.includes("/api/contact/")) {
        try {
