...
<template>
  ...
  >
    ...
    >
      <!-- Brand Logo -->
      <NuxtLink to="/" class="flex items-center h-10 w-24">
        <NuxtImg 
          src="/images/pepe.png" 
          alt="PERF Logo" 
        ...
        <div class="relative flex items-center gap-[15px]">
          <template v-if="isLoggedIn">
            <NuxtLink 
              ...
              class="text-[10px] font-bold tracking-[2px] uppercase text-gray-600 hover:text-black transition-colors"
              :class="{'text-[#D05E33]': isActive('/establishment/manage-customers')}"
            >
              Dashboard
            </NuxtLink>
            <NuxtLink 
              to="/myPlan" 
              class="text-[10px] font-bold tracking-[2px] uppercase text-gray-600 hover:text-black transition-colors"
              :class="{'text-[#D05E33]': isActive('/myPlan')}"
            >
              My Plan
            </NuxtLink>
            <button 
              class="ml-2 px-6 py-2 rounded-full border border-black text-[10px] font-bold tracking-[2px] uppercase transition-all duration-300 hover:bg-black hover:text-white"
              @click="handleLogout"
            >
              Logout
            </button>
      ...
      <!-- Mobile Menu Trigger -->
      <button 
        class="lg:hidden flex flex-col gap-1.5 p-2 transition-transform active:scale-90"
        @click="toggleMenu"
      >
        <span class="w-6 h-0.5 bg-black transition-all duration-300" :class="{'rotate-45 translate-y-2': isMenuOpen}"></span>
        ...
        <div class="flex flex-col items-center gap-10 w-full">
          ...
          <template v-else>
            <NuxtLink 
              v-if="route.path !== '/'"
              to="/auth/login" 
              class="w-full max-w-xs py-4 bg-black text-white text-center font-bold uppercase tracking-[4px] rounded-xl"
              @click="closeMenu"
            >
              Login
            </NuxtLink>
