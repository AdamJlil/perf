<template>
  <div :id="containerId" class="p-10">
    <div
      :id="titleId"
      class="flex flex-col justify-items-start items-center p-4 sm:flex sm:flex-row sm:justify-items-start sm:items-center sm:p-4"
    >
      <h1 class="uppercase text-xs lg:text-2xl font-bold tracking-0.2 text-gray-800 sm:text-base md:text-xl">
        {{ mainTitle }}
      </h1>
    ...
    <div :id="contentId" class="w-full">
      <div
        class="w-full rounded-10 border-1 border-gray-800 flex h-auto flex flex-col justify-around sm:flex-row sm:justify-normal sm:h-[30vh] md:h-[35vh] lg:h-[45vh] gap-5 sm:p-10 sm:pr-0 p-5"
      >
        <div id="content-text" class="w-full sm:w-2/5 flex flex-col justify-center">
          <transition name="fade" mode="out-in" appear>
            <p :key="displayedItem.id + '-price'" class="uppercase text-red-700 text-center sm:text-start">
              {{ displayedItem.price }}
            </p>
          </transition>
          ...
          <transition name="fade" mode="out-in" appear>
            <h1
              :key="displayedItem.id + '-title'"
              class="font-bold sm:text-sm md:text-lg lg:text-lg text-sm text-center sm:text-start"
            >
              {{ displayedItem.title }}
            </h1>
          </transition>
