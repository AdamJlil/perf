...
<template>
  <header class="color-white absolute top-0 left-0 right-0 z-50 mt-[30px]" style="font-family: Montserrat;">
    <div class="p-3 w-full flex items-center justify-between pr-[30px]">
      ...
      <div 
        :class="['fixed h-full top-0 left-0 bottom-0 max-md:w-full w-[350px] bg-[#585050] z-40 transform transition-transform duration-300 ease-in-out h-[90vh]', isMenuOpen ? 'translate-x-0' : '-translate-x-full']"
      >
        <nav class="flex flex-col px-[25px] pt-[60px] relative">
          ...
          <template v-if="currentUser">
            <NuxtLink v-if="!isAdmin && isWorkoutProgram" to="/privateWorkoutProgram" class="mb-[10px]" :class="{ 'font-bold': isActive('/privateWorkoutProgram'), 'font-light': !isActive('/privateWorkoutProgram') }" @click="closeMenu">Your Workout Programme</NuxtLink>
            <NuxtLink v-if="!isAdmin && isNutritionProgram" to="/privateNutritionProgram" class="mb-[10px]" :class="{ 'font-bold': isActive('/privateNutritionProgram'), 'font-light': !isActive('/privateNutritionProgram') }" @click="closeMenu">Your Nutrition Programme</NuxtLink>
            <NuxtLink v-if="isAdmin" to="/admin" class="py-2 mb-[10px] font-semibold" @click="closeMenu">Admin</NuxtLink>
            <button
          ...
          <template v-else>
            <NuxtLink to="/" class="py-2 mt-[30px] mb-[10px]" :class="{ 'font-bold': isActive('/'), 'font-light': !isActive('/') }" @click="closeMenu">HOME</NuxtLink>
            <NuxtLink to="/workoutPrograms" class="py-2 mb-[10px]" :class="{ 'font-bold': isActive('/workoutPrograms'), 'font-light': !isActive('/workoutPrograms') }" @click="closeMenu">WORKOUT PROGRAMS</NuxtLink>
            <NuxtLink to="/" class="py-2 mb-[10px]" :class="{ 'font-bold': isActive('/##'), 'font-light': !isActive('/##') }" @click="closeMenu">NUTRITION PLANS</NuxtLink>
            <NuxtLink to="/" class="py-2 mb-[10px]" :class="{ 'font-bold': isActive('/##'), 'font-light': !isActive('/##') }" @click="closeMenu">OUR PRODUCTS</NuxtLink>
          </template>
        </nav>
      </div>
    </div>
