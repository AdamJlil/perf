<template>
  ...
  >
    ...
    <div
      v-if="isTextInput"
      class="w-full max-w-[90%] md:max-w-[500px] h-fit flex flex-col justify-center items-center z-2"
    >
    ...
    <div v-else class="max-w-[90%] md:max-w-[500px] h-fit flex flex-col justify-center items-start z-2">
      <div v-for="(option, index) in optionRefs" :key="index" class="flex items-center gap-4 mb-4">
        <input :id="`${id}-${index}`" v-model="selectedIndex" type="radio" :value="index" class="hidden" />
        <label :for="`${id}-${index}`" class="flex items-center gap-[15px] cursor-pointer">
