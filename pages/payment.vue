      ...
      <div class="w-full grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div class="lg:col-start-2 lg:col-span-7 space-y-8">
          <div class="bg-white/40 backdrop-blur-md p-10 rounded-[30px] shadow-sm border border-white/20">
            <h2 class="text-xl font-normal mb-10 uppercase tracking-[4px] text-gray-700">Delivery Information</h2>

            <!-- Alerts -->
            <Transition name="fade">
              <div
                v-if="sendingInformation"
                class="mb-8 p-4 bg-blue-50 text-blue-700 text-xs font-medium tracking-[1px] rounded-md border border-blue-100 flex items-center gap-3 italic"
              >
                <svg
            ...
            <Transition name="fade">
              <div
                ...
                class="mb-8 p-4 bg-green-50 text-green-700 text-xs font-medium tracking-[1px] rounded-md border border-green-100 italic"
              >
            ...
            <Transition name="fade">
              <div
                v-if="form.error"
                class="mb-8 p-4 bg-red-50 text-red-600 text-xs font-medium tracking-[1px] rounded-md border border-red-100 italic"
              >
                ** {{ form.error }}
              </div>
            </Transition>
            ...
            <form class="space-y-8" @submit.prevent="handlePayment">
              <div class="relative group">
                ...
                <input
                  ...
                  placeholder="Street Name, Number, Area"
                  required
                />
                ...
                <div class="mt-6">
                  <Transition name="fade" mode="out-in">
                    <div
                      ...
                      class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100 space-y-4"
                    >
                      <div class="space-y-2">
                        <p class="text-sm text-gray-600">
                          <span class="font-bold text-gray-800 uppercase tracking-wide">Account:</span> PERF FITNESS &
                          WELLNESS
                        </p>
                        <p class="text-sm text-gray-600">
                          <span class="font-bold text-gray-800 uppercase tracking-wide">Details:</span> 011 590
                          0000112000002178 72
                        </p>
                        <p class="text-sm text-gray-600">
                          <span class="font-bold text-gray-800 uppercase tracking-wide">IBAN:</span> MA64 0115 9000 0011
                          2000 0021 7872
                        </p>
                        <p class="text-sm text-gray-600">
                          <span class="font-bold text-gray-800 uppercase tracking-wide">BIC:</span> BMCEMAMC
                        </p>
                      </div>
                    </div>
                    <div
                      v-else-if="form.paymentMethod === 'cash'"
                      class="bg-gray-50/50 p-6 rounded-2xl border border-gray-100"
                    >
                      <p class="text-sm text-gray-600 leading-relaxed italic">
                        "Payment will be collected upon delivery of your PERF equipment box at the specified address."
                      </p>
                    </div>
                  </Transition>
                </div>
              ...
              <div class="pt-10">
                <button
                  type="submit"
                  class="w-full uppercase border-2 border-black text-black bg-transparent py-5 px-6 font-bold tracking-[4px] rounded-xl hover:bg-black hover:text-white transition-all duration-300 disabled:opacity-50 active:scale-95 shadow-md"
        ...
        <div class="lg:col-span-3 space-y-8">
          <div
            v-if="selectedPlan"
            class="bg-white/60 backdrop-blur-md p-8 rounded-[30px] shadow-sm border border-white/30"
          >
...
const validateForm = () => {
  let isValid = true;
  form.errors = { address: "", city: "", phone: "", paymentMethod: "" };

  if (!form.address.trim() || form.address.trim().length < 5) {
    form.errors.address = "Please enter a valid shipping address";
    isValid = false;
  }
