...
const confirmCancelPlan = async () => {
  ...
  try {
    const response = await $fetch<any>("/api/users/request-cancel", {
      method: "POST",
    });
    ...
    toast.error(error.statusMessage || "Failed to send cancellation request");
  } finally {
...
const undoCancelRequest = async () => {
  ...
  try {
    const response = await $fetch<any>("/api/users/undo-cancel", {
      method: "POST",
    });
    ...
    toast.error(error.statusMessage || "Failed to withdraw request");
  } finally {
...
<template>
  <div class="w-full min-h-screen bg-[#EFEFEC] pt-12 pb-20 px-4" style="font-family: Montserrat">
    ...
    <div class="max-w-6xl mx-auto">
      ...
      <div
        v-if="currentPlan"
        class="bg-white/60 backdrop-blur-md p-10 lg:p-12 rounded-[40px] shadow-sm border border-white/40 mb-16 relative overflow-hidden"
      >
        <div
          class="absolute top-0 right-0 w-96 h-96 bg-[#D05E33]/5 rounded-full -translate-y-48 translate-x-48 blur-3xl"
        ></div>
        ...
        <div class="relative z-10 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-10">
          <div class="space-y-4">
            <span
              ...
              class="px-4 py-1.5 bg-[#D05E33] text-white text-[10px] font-bold uppercase tracking-[2px] rounded-full shadow-sm"
              >Active Subscription</span
            >
            <span
              v-else-if="!isPaid && !isCancelled"
              class="px-4 py-1.5 bg-orange-500 text-white text-[10px] font-bold uppercase tracking-[2px] rounded-full shadow-sm animate-pulse"
              >Pending Activation (Check Email)</span
            >
          ...
          <div v-if="isPaid" class="flex flex-wrap gap-4">
            <button
              class="uppercase border-2 border-black text-black bg-transparent py-4 px-10 rounded-xl font-bold tracking-[2px] hover:bg-black hover:text-white transition-all duration-500 text-[10px] active:scale-95"
              @click="scrollToPlans"
            >
              Change Plan
            </button>
            <button
              class="uppercase border-2 border-red-100 text-red-400 bg-transparent py-4 px-10 rounded-xl font-bold tracking-[2px] hover:bg-red-50 hover:border-red-200 transition-all duration-500 text-[10px] active:scale-95"
          ...
          <div v-else class="max-w-xs text-right">
            <p class="text-xs font-bold text-[#D05E33] uppercase tracking-[2px] leading-relaxed">
              Waiting for proceeding payment. Please check your email for the bank details or cash delivery
              instructions.
            </p>
          </div>
        </div>
      ...
      >
        <div class="bg-white p-10 rounded-[40px] max-w-md w-full shadow-2xl text-center">
          ...
          <p class="text-sm text-gray-500 mb-10">
            You are about to request an upgrade to the
            <span class="font-bold text-[#D05E33]">{{ selectedUpgradePlan }}</span> plan.
          </p>
          <div class="flex gap-4">
            <button
              class="flex-1 border border-gray-300 py-4 rounded-xl uppercase text-[10px] font-bold tracking-[2px] transition-all active:scale-95 hover:bg-gray-50"
              @click="showUpgradeModal = false"
            >
              Cancel
            </button>
            <button
              class="flex-1 bg-[#D05E33] text-white py-4 rounded-xl uppercase text-[10px] font-bold tracking-[2px] transition-all active:scale-95 shadow-lg hover:bg-[#b84d2a]"
    ...
    <Transition name="fade">
      ...
      >
        <div class="bg-white p-10 rounded-[40px] max-w-md w-full shadow-2xl text-center border-2 border-red-50">
          ...
          <h2 class="text-xl font-bold uppercase tracking-[2px] text-gray-800 mb-4">Confirm Cancellation</h2>
          <p class="text-sm text-gray-500 mb-10 leading-relaxed">
            Are you sure you want to request a subscription cancellation? This action is subject to approval.
          </p>
          <div class="flex flex-col gap-3">
            <button
              class="w-full h-[50px] bg-black text-white py-4 rounded-xl uppercase text-[10px] font-bold tracking-[3px] transition-all hover:bg-gray-800 active:scale-95 shadow-lg"
    ...
    <Transition name="slide-down">
      <div
        ...
        class="fixed top-10 left-1/2 -translate-x-1/2 z-[1200] bg-black text-white px-8 py-4 rounded-full shadow-2xl flex items-center gap-4 border border-white/10"
      >
